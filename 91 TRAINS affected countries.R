...
rm(ricardodev, ricardodev.readonly, ricardomain, ricardomain.readonly, mail, ft.api, devtwitter, bastiat, nyt.api, grid)
...
t <- master.sliced %>% select(mast.chapter, mast.id, affected.flow) %>% unique()
t_2 <- master.sliced %>% 
  filter(mast.id == "L") %>%
...
necessary.dataframes <- list(gta_it_revised,
                             gta_affected_tariff_line,
                             gta_tariff_line,
                             gta_affected_jurisdiction,
                             gta_implementing_jurisdiction,
                             gta_distorted_market,
                             gta_support_goods_trade,
                             gta_support_fta)
rm(list = ls())
...
SET @year =
  (
    SELECT YEAR(announcement_date)
    FROM gta_measure gm
    WHERE gm.id = @measure_id
  );
END IF;
SET @year = @year - 1;
...
IF (@dm_freeze = 1) THEN
UPDATE gta_distorted_market
SET type = 'A'
WHERE intervention_id = @intervention_id
AND type = 'N';
END IF;
IF (@aj_freeze = 1) THEN
UPDATE gta_affected_jurisdiction
SET type = 'A'
WHERE intervention_id = @intervention_id
AND type = 'N';
END IF;
...
INSERT INTO gta_affected_jurisdiction
(
  select distinct @intervention_id, g2.partner_jurisdiction_id, 'D' 
  from (select distinct g2.reporter_jurisdiction_id, g2.partner_jurisdiction_id 
        from gta_support_goods_trade g2 
        join gta_affected_tariff_line atl on atl.intervention_id = @intervention_id and g2.year = @year and atl.tariff_line_code = g2.tariff_line_code) g2
  join (select distinct g1.reporter_jurisdiction_id, g1.partner_jurisdiction_id 
...
ON DUPLICATE KEY UPDATE
type = IF(gta_affected_jurisdiction.type = 'A', 'N', 'D');
END IF;
END IF;
...
SET is_positively_affected =
  (
    SELECT EXISTS (
      SELECT NULL
      FROM
      gta_support_service_trade gsst
      WHERE reporter_jurisdiction_id IN
      (
        SELECT jurisdiction_id
        FROM gta_implementing_jurisdiction
        WHERE intervention_id = @intervention_id
      )
      AND gsst.sector_code = ac.sector_code
      AND year = @year
    )
  )
WHERE intervention_id = @intervention_id;
...
SET is_positively_affected =
  (
    SELECT EXISTS (
      ...
      FROM
      gta_support_service_trade gsst
      WHERE partner_jurisdiction_id IN
      (
        SELECT jurisdiction_id
        FROM gta_implementing_jurisdiction
        WHERE intervention_id = @intervention_id
...
(
  ...
  (
    ...
    (
      SELECT DISTINCT g.reporter_jurisdiction_id
      FROM gta_support_service_trade g
      WHERE g.year = @year
      AND g.partner_jurisdiction_id IN
      (
        SELECT jurisdiction_id
      ...
      AND g.sector_code IN
      (
        SELECT sector_code
        FROM gta_affected_sector ac
        WHERE ac.intervention_id = @intervention_id
      )
    )
  )
);
...
(
  ...
  (
    ...
    (
      SELECT DISTINCT g2.partner_jurisdiction_id
      FROM
      gta_support_service_trade g1,
      gta_support_service_trade g2,
      gta_affected_sector ac,
      gta_implementing_jurisdiction ij,
      gta_distorted_market dm
      WHERE g1.year = @year
      AND g2.year = @year
      AND ac.intervention_id = @intervention_id
      AND ij.intervention_id = @intervention_id
      AND dm.intervention_id = @intervention_id
      AND ac.sector_code = g1.sector_code
      AND ac.sector_code = g2.sector_code
      AND g1.partner_jurisdiction_id = ij.jurisdiction_id
      AND g1.reporter_jurisdiction_id = dm.jurisdiction_id
      AND g1.reporter_jurisdiction_id = g2.reporter_jurisdiction_id
      AND dm.type = 'N'
    )
  )
);
...
(
  ...
  (
    ...
    FROM gta_affected_sector ac
    WHERE ac.intervention_id = @intervention_id
    AND ac.sector_code = g.sector_code
  )
);
...
(
  ...
  (
    ...
    FROM gta_affected_sector ac
    WHERE ac.intervention_id = @intervention_id
    AND g.sector_code = ac.sector_code
  )
);
...
(
  ...
  (
    ...
    (
      SELECT DISTINCT g2.partner_jurisdiction_id
      FROM
      gta_support_investment g1,
      gta_support_investment g2,
      gta_affected_sector ac,
      gta_implementing_jurisdiction ij,
      gta_distorted_market dm
      WHERE g1.year = @year
...
(
  ...
  (
    ...
    (
      SELECT DISTINCT g2.partner_jurisdiction_id
      FROM
      gta_support_migration g1,
      gta_support_migration g2
      WHERE g1.year = @year
      AND g2.year = @year
      AND EXISTS
      (
      ...
      (
        ...
        FROM gta_distorted_market dm
        WHERE dm.intervention_id = @intervention_id
        AND g2.reporter_jurisdiction_id = dm.jurisdiction_id
        AND type = 'N'
      )
      AND g1.reporter_jurisdiction_id = g2.reporter_jurisdiction_id
    )
...
(
  ...
  (
    ...
    FROM gta_affected_jurisdiction aj
    WHERE aj.intervention_id = @intervention_id
    AND g2.partner_jurisdiction_id = aj.jurisdiction_id
    AND type = 'N'
  )
  AND g1.reporter_jurisdiction_id = g2.reporter_jurisdiction_id
);
