...
columns <- c("countryImposingNTMs", "affectedCountriesNames","ntmCode","hsCode",
             ...
             "repealDate", "regulationFile", "measureDescription")
...
for(page in 31:100){
  ...
  for(i in 0:(rows-1)){
    ...
    cell <- table %>% html_elements(paste0("[row-index='",i,"']"))%>%
      .[[2]] %>% html_elements(paste0("[col-id='",columns[col],"']")) %>%
      html_text2() 
    
    row <- c(row, cell)
    }
    
    #row <- c(page, i, unlist(strsplit(row, split = "\n"))) #reformat and add to table
    UNCTAD <- rbind(UNCTAD,row)
