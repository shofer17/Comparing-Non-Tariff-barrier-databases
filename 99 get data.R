...
table <- table$getPageSource() #
test <- table %>% .[[1]] %>% read_html()
#test2 <- read_html(x = as(test, "character"))
t <- html_elements(test, 'ag-grid-angular')
#t3 <- html_elements(t, "div.ag-cell.ag-cell-not-inline-editing.ag-cell-auto-height.ag-cell-value")

cols = 4
UNCTAD <- data.frame(ncol = cols)

for(i in 1:20){
  row <- t %>% html_elements(paste0("[row-index='",i,"']"))%>%
    .[[2]] %>%
    html_text2()
...
t2 <- html_elements(t, ".ag-row.ag-row-no-focus.ag-row-level-0.ag-row-position-absolute")
t3 <- html_elements(t,"[row-index='0']")
t3[[2]] %>% html_text2()

t3 <- html_elements(t2, ".ag-cell.ag-cell-not-inline-editing.ag-cell-auto-height.ag-cell-value")
